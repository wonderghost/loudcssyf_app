        ...
        try {
            ...
            foreach($data as $key => $value) {
                ...
                $all[$key] = [
                    ...
                    'comission' =>  $value->comission,
                    'telephone_client'  =>  $value->telephone_client,
                    'pdraf' =>  $value->pdrafUser()->only('localisation','username','nom','prenom'),
                    'pdc_hote'  =>  $value->pdrafUser()->pdcUser() ? $value->pdrafUser()->pdcUser()->usersPdc()->only('localisation','username') : '-',
                    'marge' =>  $marge,
                    'total' =>  $marge + $value->comission,
                    'created_at'    =>  $created_at->toDateString(),
                    'hour'  =>  $created_at->toTimeString(),
    ...
    public function confirmReaboAfrocash(Request $request) {
        try {
            ...
            if($reabo_afrocash->update()) {
                if($reabo_afrocash->upgrade()->first())
                {
                    // c'est un upgrade , aucun message n'est recu
                    return response()->json('done',200);
                }
                else
                {
                    // c'est un reabonnement classique
                    $msg = "Bonjour, votre abonnement: ".$reabo_afrocash->serial_number." est activé pour ".$reabo_afrocash->duree." mois , à la formule ".$reabo_afrocash->formule_name." (".number_format($reabo_afrocash->montant_ttc)." GNF)".".\n Merci pour votre fidelite.";
