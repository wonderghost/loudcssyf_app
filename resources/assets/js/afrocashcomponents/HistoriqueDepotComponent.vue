<template>
    <div class="uk-container uk-container-large">
        ...
        <ul class="uk-breadcrumb">
            ...
            <li><span>Historique de Depot</span></li>
        </ul>

        <h3>Historique de Depot</h3>
        <hr class="uk-divider-small">

        <nav uk-navbar>
            <div class="uk-navbar-left">
                <ul class="uk-navbar-nav">
                    <li class="uk-button"><router-link to="/afrocash/historique/retrait">Historique de Retrait</router-link></li>
                </ul>
            </div>
        </nav>
        ...
        <div>
            <table class="uk-table uk-table-small uk-table-divider uk-table-striped uk-table-hover">
                ...
                <tbody>
                    <tr v-for="(d,index) in depots" :key="index">
                        <td>{{ d.date }}</td>
                        <td>{{ d.initiateur }}</td>
                        <td>{{ d.destinateur }}</td>
                        <td>{{ d.montant | numFormat }}</td>
                        <td>{{ d.comission | numFormat}}</td>
                        <td v-if="d.status" class="uk-text-success"><span uk-icon="check"></span></td>
                        <td v-else class="uk-text-primary"><span uk-icon="more"></span></td>
                        <td v-if="d.pay_state" class="uk-text-success"><span uk-icon="check"></span></td>
                        <td v-else class="uk-text-primary"><span uk-icon="more"></span></td>
                    </tr>
                </tbody>
            </table>
    ...
    export default {
        ...
        methods : {
            onInit : async function () {
                try {
                    this.isLoading = true
                    let response = await axios.get('/user/afrocash/historique/depot')
                    if(response) {
                        this.depots = response.data.all
