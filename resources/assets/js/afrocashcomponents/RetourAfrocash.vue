        ...
        <div class="uk-container uk-container-large">
           ...
           <div class="uk-grid-small uk-grid-divider" uk-grid>
               <div class="uk-width-1-3@m">
                   ...
                   <form @submit.prevent="sendRetourRequest()">
                ...
                <div class="uk-width-1-3@m">
                    <div class="uk-card uk-card-default uk-border-rounded" style="box-shadow : none !important ; border : solid 1px #ddd !important;">
                        ...
                        <div class="uk-card-body uk-text-center">
                            <span class=" uk-card-title">{{ afrocashAccount.solde | numFormat }} GNF</span>
                        </div>
                    </div>                
                </div>
            <!-- // -->
    ...
    export default {
        ...
        methods : {
            onInit : async function () {
                try 
                {
                    let response = await axios.get('/afrocash/retour-afrocash')
                    if(response)
                    {
                        this.users = response.data.users
                        this.afrocashAccount = response.data.account
                    }
                }
                catch(error)
                {
            ...
            sendRetourRequest : async function () {
                try {
                    this.isLoading = true
                    this.errors = []
                    this.dataForm._token = this.myToken
                    let response = await axios.post('/afrocash/retour-afrocash',this.dataForm)
