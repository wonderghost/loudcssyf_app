<template>
    <div class="uk-container uk-container-large">
        ...
        <h3 class="uk-margin-top">Upgrade</h3>
        ...
        <div class="uk-grid-small" uk-grid>
            <div class="uk-width-2-3@m">
                ...
                <form @submit.prevent="sendUpgradeRequest()" class="uk-grid-small" uk-grid>
                    ...
                    <div class="uk-width-1-3@m">
                        <template v-if="Object.keys(upgradeData).length === 0">
                            <div class="uk-width-1-1@m">
                                ...
                                <select v-model="form.old_formule" class="uk-select uk-border-rounded">
                                    <option value="">-- Selectionnez une formule --</option>
                                    <option :data-prix="f.prix" :value="f.nom" v-for="(f,index) in formules" :key="index">{{ f.nom }}</option>
                                </select>
                            </div>
                        </template>
                        <template v-else>
...
import 'vue-loading-overlay/dist/vue-loading.css';
    ...
    export default {
        ...
        methods : {
            ...
            sendUpgradeRequest : async function () {
                try {
                    this.isLoading = true
                    this.errors = []
                    this.form._token = this.myToken
                    let response = await axios.post('/vente/upgrade',this.form)
            ...
            setMontant : function () {
                try {
                    var ttc = 0
                    if(this.actifFormule && this.oldFormule) {
                        ttc = ((this.actifFormule.prix - this.oldFormule.prix) + this.totalOption()) * this.form.duree

                        if(ttc < 0) {
                            this.form.formule = ""
                            throw "upgrade impossible."
                        }
                    }
            ...
            checkUpgrade : async function () {
                try {
                    let response = await axios.post('/user/rapport/check-upgrade',{
                        _token : this.myToken,
                        serial_number : this.form.materiel
                    })

                    if(response && response.data != 'fail') {
                        this.upgradeData = response.data
                        this.form.old_formule = response.data.formule_name
        ...
        computed : {
            oldFormule() {
                let data = this.formules.filter((f) => {
                    return f.nom == this.form.old_formule
                })
                if(data) {
                    return data[0]
                }
